 ...
 * サービス情報取得APIを実行する関数
 * @param なし
 * @return {Promise<AxiosResponse<Model.GetServiceInfoApiResponse>} - Promise型のAPI実行結果を返却する
 */

export const getServiceInfoApi = (): Promise<
    AxiosResponse<Model.GetServiceInfoApiResponse>
> => {
    ...
    return new Promise((resolve, reject) => {
        axios(requestConfig)
            .then((res: AxiosResponse<Model.GetServiceInfoApiResponse>) => {
...
/**
 * カードアクセストークン発行APIを実行する関数
 * @param {Model.IssueAuthTokenApiParams} args - ユーザーが入力したカード番号とPIN番号
